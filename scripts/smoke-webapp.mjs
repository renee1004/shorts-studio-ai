...
await command("creation", { ...importInput, suppliedText: "바뀐 문장\n둘째 문장" }, 409);
await command("creation", { ...importInput, operationId: crypto.randomUUID(), suppliedText: "한 문단" }, 400);
for (const stage of ["research", "angles", "script"]) {
  const saved = await command(`projects/${imported.project.id}/prepare`, { stage });
  assert.equal(saved.latestScript.id, imported.latestScript.id);
  assert.equal(saved.angles.length, 0, "Imported scripts must not generate unused angles");
}
const importedPage = await fetch(`${base}/create?project=${imported.project.id}`, { headers: { cookie } });
assert.equal(importedPage.status, 200);
const importedHtml = await importedPage.text();
assert.ok(!importedHtml.includes('"digest":"'));
assert.ok(importedHtml.includes("입력한 대본을 저장했습니다"), "Resume must show the saved script");
const notes = await command("creation", { mode: "notes", topic: "정리 자료 가져오기", operationId: crypto.randomUUID(), suppliedText: "가".repeat(4000) }, 201);
...
console.log("OK imported script, notes, concurrent replay and resume without regeneration");
