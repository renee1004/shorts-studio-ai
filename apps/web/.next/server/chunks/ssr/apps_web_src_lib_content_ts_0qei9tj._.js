...
3. 다음 편에서 바꿀 것 3가지를 우선순위대로 제안해줘. (한 번에 하나만 바꿀 수 있게)`}],checkpoints:["9:16 영상 파일이 손에 들어왔다","내레이션 음성이 자막과 어긋나지 않는다","소리를 끄고 봐도 내용이 전달된다","AI 산출물 외에 내 원본 요소가 최소 하나 들어갔다","제목·설명·해시태그가 채워져 업로드됐다"],pitfalls:[{title:"Short 포맷의 제약을 먼저 확인하세요",body:"출시 시점 기준 Short는 약 60초 고정, 세로 전용, 영어 내레이션만, 만 18세 이상, 대본 편집과 타임라인 수정 불가입니다. 조정 수단은 포커스 프롬프트를 바꿔 재생성하는 것뿐입니다. 한국어 쇼츠라면 Explainer + 무료 편집기 경로가 현실적입니다."},{title:"CapCut 자동화는 공식 경로가 없습니다",body:"CapCut의 Open Platform은 에디터 안에서 돌아가는 플러그인용이고, JSON을 보내면 영상을 돌려주는 렌더링 API는 공개되지 않았습니다. 프로젝트 파일(draft_content.json)을 코드로 만드는 우회법이 있지만 앱 업데이트마다 깨지고 데스크톱 한 대에 묶입니다. 자동화가 목적이라면 이 프로젝트의 FFmpeg 렌더 경로가 안전합니다."},{title:"같은 템플릿으로 양산하면 수익화가 막힙니다",body:"유튜브는 반복적·대량생산 콘텐츠를 '진정성 없는 콘텐츠'로 보고 수익화 대상에서 제외합니다. 편당 원본 요소를 넣는 게 속도보다 중요합니다."},{title:"쇼츠 수익화 문턱은 2027년 2월 1일에 올라갑니다",body:"현재는 구독자 1,000명 + 90일간 쇼츠 조회 1,000만 회(또는 12개월 시청 4,000시간)입니다. 2027년 2월 1일부터 신규 신청자는 2,000만 회 / 8,000시간으로 두 배가 됩니다. 기존 파트너는 유지되니, 지금 시작해 문턱을 먼저 넘는 편이 유리합니다."}]}],c=b.flatMap(a=>a.prompts.map(b=>({...b,stepSlug:a.slug,stepTitle:a.title,stepOrder:a.order}))),d=b.flatMap(a=>a.tasks.map(a=>a.id));[...[{id:"prep-google",title:"구글 계정",detail:"notebooklm.google.com 에 로그인하면 바로 시작됩니다. 별도 신청이나 대기 목록이 없습니다.",cost:"무료"},{id:"prep-notebooklm",title:"NotebookLM 무료 플랜",detail:"노트북 수, 노트북당 소스 수, 하루 생성 횟수에 한도가 있습니다. 한도는 자주 바뀌므로 Studio 패널에 표시되는 남은 횟수를 기준으로 계획하세요.",cost:"무료"},{id:"prep-youtube",title:"유튜브 채널",detail:"구글 계정으로 개설됩니다. 채널명은 3단계에서 확정하니 지금은 만들지 않아도 됩니다.",cost:"무료"},{id:"prep-editor",title:"무료 편집 도구 1개",detail:"한국어 쇼츠를 만들려면 9:16 조립용 도구가 필요합니다. Vrew(한국어 자동 자막·TTS)나 CapCut 무료 플랜 중 하나만 고르세요.",cost:"무료 플랜"},{id:"prep-canva",title:"썸네일·채널 아트 도구",detail:"Canva 무료 플랜으로 프로필 이미지와 배너, 커버 프레임을 만듭니다.",cost:"무료 플랜"}].map(a=>a.id),...d];let e=b.reduce((a,b)=>a+b.minutes,0);a.s(["allPrompts",0,c,"deviceRoles",0,[{device:"모바일",role:"리서치와 검수",detail:"NotebookLM 앱에서 프롬프트를 실행하고 소스를 담습니다. 자동 렌더링 화면에서 자막이 넘어가는 속도를 눈으로 확인하고, 완성된 영상은 유튜브 앱으로 업로드합니다. 영상 렌더링만 못 합니다."},{device:"노트북·데스크톱",role:"대본 확정과 렌더링",detail:"대본을 다듬고 렌더 스펙을 내려받아 명령 한 줄로 mp4를 만듭니다. FFmpeg와 Node만 있으면 되고, 어느 컴퓨터에서 실행해도 같은 결과가 나옵니다."},{device:"기기 사이 이동",role:"진행 상황 코드",detail:"체크 상태는 기기마다 따로 저장되므로, 아래 코드나 링크로 옮깁니다. 노트북과 소스는 구글 계정에 붙어 있어서 로그인만 하면 그대로 이어집니다."}],"getAdjacentSteps",0,function(a){let c=b.findIndex(b=>b.slug===a);return -1===c?{}:{prev:b[c-1],next:b[c+1]}},"getStep",0,function(a){return b.find(b=>b.slug===a)},"realityChecks",0,[{title:"세로 쇼츠 자동 생성은 아직 영어 전용입니다",body:"NotebookLM의 Short 포맷(약 60초, 9:16)은 출시 시점 기준 영어 내레이션만 지원하고 만 18세 이상, 유료 플랜 우선 제공 후 무료로 확대되는 중입니다. 한국어로 만들려면 80개 이상 언어를 지원하는 Explainer 포맷이나 오디오 개요로 재료를 만들고, 무료 편집기에서 9:16으로 조립하는 경로를 씁니다. 5단계에 두 경로가 모두 정리되어 있습니다.",tone:"warn"},{title:"'무료로 돈 복사'는 현실의 문턱을 건너야 합니다",body:"제작 비용은 실제로 0원에 가깝게 만들 수 있지만, 광고 수익을 받으려면 구독자 1,000명과 90일간 쇼츠 조회 1,000만 회(또는 12개월 시청 4,000시간)를 넘어야 합니다. 이 조건은 2027년 2월 1일부터 신규 신청자에게 2,000만 회 / 8,000시간으로 올라갑니다.",tone:"warn"},{title:"AI 양산은 수익화 거절 사유입니다",body:"유튜브는 템플릿을 반복한 대량생산·저노력 콘텐츠를 '진정성 없는 콘텐츠'로 분류해 수익화에서 제외합니다. AI로 속도를 얻되, 편당 내 관점·목소리·직접 만든 예시를 반드시 얹으세요.",tone:"warn"},{title:"그래도 이 파이프라인이 남기는 것",body:"리서치·기획·대본·영상 생성을 한 도구 안에서 끝내면 편당 제작 시간이 크게 줄고, 소스에 근거한 인용이 남습니다. 수익화 문턱을 넘기 전이라도 제휴 링크, 전자책, 상담 같은 다른 수익 경로를 같은 자산으로 열 수 있습니다.",tone:"info"}],"steps",0,b,"totalMinutes",0,e])}];

//# sourceMappingURL=apps_web_src_lib_content_ts_0qei9tj._.js.map