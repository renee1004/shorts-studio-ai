        ...
        <div className="min-w-0">
          ...
          {step.slug === "studio-render" && (
            <section className="mt-12 rounded-2xl border border-primary/30 bg-primary/[0.07] p-5 sm:p-6">
              ...
              <div className="mt-4">
                <LinkButton href="/playbook/render" className="font-semibold">
                  자동 렌더링 열기
                </LinkButton>
              </div>
            </section>
          ...
          <nav className="mt-12 mb-4 flex flex-col gap-3 border-t border-border/70 pt-6 sm:flex-row sm:justify-between">
            {prev ? (
              <LinkButton href={`/playbook/steps/${prev.slug}`} variant="ghost" className="justify-start">
                ← {prev.order}단계. {prev.title}
              </LinkButton>
            ) : (
              <LinkButton href="/playbook" variant="ghost" className="justify-start">← 전체 흐름으로</LinkButton>
            )}
            {next ? (
              <LinkButton
                href={`/playbook/steps/${next.slug}`}
                className="justify-start font-semibold sm:justify-center"
              >
                {next.order}단계. {next.title} →
              </LinkButton>
            ) : (
              <LinkButton href="/playbook/prompts" variant="outline">프롬프트 모음 보기 →</LinkButton>
            )}
          </nav>
        </div>
      </div>
