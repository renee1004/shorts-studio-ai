...
export default async function FactoryDetailPage({
  ...
  const detail = await context.run(({ db }) => loadRenderDetail(db, workspace.id, renderId));

  return (
    <div className="mx-auto w-full max-w-5xl space-y-6">
      <header>
        <p className="font-mono text-[11px] tracking-widest text-muted-foreground">VIDEO FACTORY</p>
