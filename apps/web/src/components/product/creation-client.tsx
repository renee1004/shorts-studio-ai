...
import { CreationStart } from "./creation-start";
...
export function CreationClient({
  ...
  const [showForm, setShowForm] = useState(
    Boolean(initialProject || initialImport),
  );
    ...
    <div className="mx-auto w-full max-w-3xl space-y-7 py-6">
      <header>
        ...
        <p className="mt-3 text-muted-foreground">
          {mode === "script"
            ? "영상 제목과 대본을 확인한 뒤 저장하세요."
            : "AI가 대본을 준비합니다. 실행 시 API 비용이 발생할 수 있어요."}
        </p>
      </header>
      <CreationSteps />
      {!started && (
      ...
      {(active || projectId || error) && (
        <details
          open={Boolean(active) || undefined}
          className="rounded-2xl border p-5"
        >
          <summary className="cursor-pointer font-semibold">
            {active ? `대본 준비 중 · ${elapsed}초` : "진행 내역 보기"}
          </summary>
