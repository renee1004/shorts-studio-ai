...
export async function handleWorkerRequest(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  if (req.method === "POST" && url.pathname === "/internal/render") {
    ...
    res.end(JSON.stringify({ accepted: true, renderJobId: payload.renderJobId }));
