...
const serverEnvSchema = z.object({
  ...
  AUTH_SESSION_SECRET: z.string().min(16).default("demo-session-secret-change-me"),
...
function parseFlagOverride(raw: string | undefined): Record<string, boolean> {
  ...
  try {
    ...
    throw new EnvValidationError(["FEATURE_FLAGS_OVERRIDE가 올바른 JSON이 아닙니다."]);
  }
  return {};
}
...
export function loadServerEnv(source: NodeJS.ProcessEnv = process.env): ServerEnv {
  ...
  if (env.AUTH_PROVIDER === "supabase") {
    if (!env.SUPABASE_URL) issues.push("AUTH_PROVIDER=supabase에는 SUPABASE_URL이 필요합니다.");
    if (!env.SUPABASE_ANON_KEY)
      issues.push("AUTH_PROVIDER=supabase에는 SUPABASE_ANON_KEY가 필요합니다.");
  }
  if (env.APP_MODE === "live" && !env.YOUTUBE_API_KEY) {
    issues.push("APP_MODE=live에는 YOUTUBE_API_KEY가 필요합니다. demo 모드로 두면 키 없이 실행됩니다.");
  }
  if (env.NODE_ENV === "production" && env.AUTH_SESSION_SECRET.startsWith("demo-session-secret")) {
    issues.push("production에서는 AUTH_SESSION_SECRET을 바꿔야 합니다.");
  }

  if (issues.length > 0) throw new EnvValidationError(issues);

  return { ...env, featureFlagsOverride: parseFlagOverride(env.FEATURE_FLAGS_OVERRIDE) };
}
